...
contract('BuyBack', accounts => {
	...
	const assertBalance = (address, expectedBalance) => {
		return coin.balanceOf(address)
			.then(balance => assert.equal(balance.toNumber(), expectedBalance))
	}

	const assertEthBalance = (address, expectedBalance) => {
		return Promise.resolve()
			.then(() => web3.eth.getBalance(address))
			.then(balance => assert.equal(Math.round(balance.toNumber() / 100000), Math.round(expectedBalance / 100000)))
	}

	const getTransactionCost = hash => {
	...
	it('should be possible to set new prices', () => {
		const newBuyPrice = 10
		const newSellPrice = 20
	...
	it('should not be possible to set prices sellPrice < buyPrice', () => {
		const newBuyPrice = 20
		const newSellPrice = 10
	...
	it('should be possible to sell tokens', () => {
		...
		return exchange.init(coin.address, delayedPayments.address)
			...
			.then(getTransactionCost)
			.then(txCost => {
				balance = balance.valueOf() - txCost * 100000000000 + sellAmount
				assertEthBalance(accounts[0], balance)
			})
			...
			.then(getTransactionCost)
			.then(txCost => {
				balance = balance.valueOf() - txCost * 100000000000
				assertEthBalance(accounts[0], balance)
			})
	...
	it('should withdraw eth, and fire WithdrawEth event', () => {
		const withdrawValue = 10
		const withdrawTo = '0x0000000000000000000000000000000000000005'
		let watcher
		return exchange.init(coin.address, delayedPayments.address)
			.then(() => {
				eventsHelper.setupEvents(exchange)
